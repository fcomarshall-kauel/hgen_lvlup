...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(
      `https://server.ai-labhelper.com/api/chat?session_id=666288c3bbdeec47501c57e2&message=${encodeURIComponent(message)}&thread_id=conversacion1&additional_information=${encodeURIComponent(additional_information)}`,
      {
        method: 'POST',
        headers: {
